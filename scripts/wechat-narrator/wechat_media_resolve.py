...
# 2026-07-13、07-15 观测)。命中则视为无效描述,重试一次,仍失败由调用方回退占位。
# 话术变体多:"无法直接访问图片链接"、"请将图片发给我/放到目录"等,故放宽匹配。
REFUSAL_RE = re.compile(
    r"无法(直接)?(访问|查看|识别|打开|分析|获取)"       # 无法(直接)访问/查看/分析…
    r"|没有(收到|看到|接收)|未(收到|提供)|看不到"
    r"|图片(链接)?无法|访问这(张|个)图|图片的链接"       # 无法访问图片链接
    r"|(请|请您|请把|请将).{0,12}(发给我|放到|上传|保存到)"  # 请把图片发给我/放到目录
    r"|重新发送")
